...
export default function AddQuestionPage({ params }: { params: { id: string } }) {
    // Note: parseInt(params.id) convertit la chaîne dynamique en nombre.
    ...
    const createQuestionMutation = useMutation({
        mutationFn: (payload: QuestionPayload) => questionService.create(quizId, payload),
        
        onSuccess: () => {
            // CORRECTION CLÉ D'INVALIDATION : 
            // Invalider la clé utilisée par la page d'édition du quiz (EditQuizPage)
            // L'EditQuizPage utilise ['quiz', quizId] pour récupérer les détails + questions.
            queryClient.invalidateQueries({ queryKey: ['quiz', params.id] }) 
            
            // Rediriger l'utilisateur vers la page d'édition du quiz.
        ...
        onError: (err: any) => {
            ...
            let errorMessage = err.response?.data?.detail || "Erreur lors de la création."
            if (err.response?.data?.options) {
                errorMessage = "Erreur options : " + err.response.data.options
            }
    ...
    const handleRemoveOption = (index: number) => {
        ...
        if (options[index].isCorrect) {
            // Assurer qu'il y a toujours une réponse correcte si on supprime la bonne
    ...
    return (
        <div className="max-w-3xl mx-auto py-10 px-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-8 bg-white p-6 rounded-lg shadow">
                ...
                {error && (
                    <div className="bg-red-50 border border-red-200 text-red-600 p-3 rounded text-center">
                        {error}
                    </div>
                )}
                {createQuestionMutation.isError && (
                    <div className="bg-red-50 border border-red-200 text-red-600 p-3 rounded text-center">
                        Erreur de l'API: {createQuestionMutation.error.message}
                    </div>
                )}
